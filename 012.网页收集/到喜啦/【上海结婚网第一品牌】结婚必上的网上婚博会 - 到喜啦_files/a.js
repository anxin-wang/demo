...
var s4Com = windowServer == "com" || windowServer == undefined || windowServer == "" ? "com" : windowServer;
var s4Net = windowServer == "com" || windowServer == undefined || windowServer == "" ? "net" : windowServer;
var dxlHttp = {
	v:"201502121359",
	...
	e:windowHttp + "//event.daoxila." + s4Com + "/",
	newCom:windowHttp + "//sh.daoxila." + s4Com + "/",
	iq:windowHttp + "//iq" + dxlfiles + ".dxlfile." + s4Com + "/"
}
...
(function($) {
	$.extend(String.prototype, {
		'isPhone': function() {
            return (new RegExp(/^(?:(?:1(?:3[4-9]|5[012789]|8[78])\d{8}|1(?:3[0-2]|5[56]|8[56])\d{8}|18[0-9]\d{8}|1[35]3\d{8})|14[57]\d{8}|170[059]\d{7}|17[67]\d{8})$/).test(this));                       
        },
		'isMail': function() {
...
(function($) {
	...
	var frameSet = [
		...
		{name:"hotelOrder",js:"order/js/hotelOrder/1501.js",css:"order/css/hotelOrder/1501.css"},
		...
		{name:"clickQuery",js:"public/js/jquery.clickQuery.js",css:""},
		{name:"miguo",js:"public/js/miguo.js",css:""},
		...
		{name:"minisite",js:"order/js/minisite/1501.js",css:"order/css/minisite/1501.css"},
		{name:"flag",js:"order/js/flag/1501.js",css:"order/css/flag/1501.css"},
		{name:"M-hotelOrder",js:"order/js/M-hotelOrder/1501.js",css:"order/css/M-hotelOrder/1501.css"},
	...
	$.extend({
		...
		"dxlCustomer":function(){
			window.open('http://chat10.live800.com/live800/chatClient/chatbox.jsp?companyID=378034&jid=3193679090&enterurl=' + encodeURIComponent(window.location.href) + '&pagetitle=' + encodeURIComponent($("title").text()) + '&firstEnterUrl=' + encodeURIComponent(window.location.href) + '&lan=zh&tm=1416906709567', 'dxl'+new Date().getTime(), 'height=520, width=720, top=0, left=0, toolbar=no, menubar=no, scrollbars=no, resizable=no, location=no, status=no');
		},
		...
		"dxlLodinShow":function(execute){
			$.getJSON(dxlHttp.my + "jsonp/?act=checkloginStatus&callback=?",function(data){
				if(data.code == 1){
					...
					typeof wmx_set_account == "function" ? wmx_set_account(data.user_authKey) : "";
				}
				$.isFunction(execute) ? execute(data) : "";
		...
		"dxlGaPageTracker":function(xnUrl,orderFrom){
			try{
				...
				//国双
				if(xnUrl.indexOf("/HunYan/Order") >= 0){
					gsTrack(xnUrl,orderFrom)
				}
			}catch(e){};
		},
		"dxlTopCall":function(_par){
			...
			html += '</div>';
			html += '<div id="dxlTopCallAlertImg1" class="dxlTopCallAlertImg clearfix"><div class="img"><img class="appImg" src="'+ dxlHttp.s4+'www/img/erweima/app.png" width="106" height="106"/>';
			html += 'iPhone客户端';
			html += '<div>扫一扫下载到手机<br/>更多下载方式</div></div>';
			html += '</div>';
			html += '<div id="dxlTopCallAlertImg2" class="dxlTopCallAlertImg clearfix"><div class="img"><img class="appImg" src="'+ dxlHttp.s4+'www/img/erweima/app.png" width="106" height="106"/>';
			html += 'Android客户端';
			html += '<div>扫一扫下载到手机<br/>更多下载方式</div></div>';
			html += '</div>';
		...
		"dxlAarrow":function(){
			$(window).width() <= 1290 ? $("body").addClass("narrow") : "";
		},
		"dxlDiaoCha":function(){
				$("#dxlDiaoCha").remove();
		...
		"dxlmg":function(dxlJson){
			...
			try{
				wmx_trackBtn(json.order_from, json.region + "," + json.price + "," + json.desk + "," + json.date);
			}catch(e){}
		},
		"dxlSmsSend":function(par,callback){
			...
			$.getJSON(dxlHttp.com + "jsonpnew/?act=sendSms&" + $.param(json) + "&callback=?",function(d){
...
$(document).ready(function(){
	if(dxlHost[2] != "evo" && dxlHost[2] != "crm" && dxlHost[2] != "cms" && dxlHost[2] != "sys2" && dxlHost[2] != "admin" && dxlHost[2] != "b" && dxlHost[2] != "cms2" && dxlHost[2] != ""){
		$.jsSequence({frame:["statistics","miguo"]});
		
	}
});

//dxlAnalytics
$.getScript(windowHttp + "//da.daoxila.com/da.php?v=" + new Date().getTime(),function(){
	try{
		//全站调用
		if(typeof dxlAnalyticsData == "string"){
			da_send_para("{type:0,data:{" + dxlAnalyticsData + "}}");
		}else{
			da_send_para("{type:0,data:null}");
		}
		//项目调用
		$.isFunction(dxlAnalyticsCallback) ? dxlAnalyticsCallback() : "";
	}catch(e){}
});
...
(function($) {
	$.extend({
		...
		"dxlHeaderNav":function(){
			//导航
			$("header nav ul li.navItem").each(function(index, element) {
				var time;
				var _this = $(this);
				var thisNavList = $("header .navList").eq(index);
				var jti = _this.find("i");
				thisNavList.append("<span></span>");
				thisNavList.find("li").append("<i></i>");
				
                _this.on({"mouseenter":mouseenterNav,"mouseleave":mouseleaveNav});
				thisNavList.on({"mouseenter":mouseenterNav,"mouseleave":mouseleaveNav});
				
				function clearTime(callback){
					thisNavList.stop();
				...
				function mouseenterNav(){
					clearTime(function(){
						jti.addClass("jt");
						thisNavList.css("left",_this.offset().left).fadeIn(200,function(){
							thisNavList.find("span").addClass("t");
						});
					})
				}
				function mouseleaveNav(){
					clearTime(function(){
						time = setTimeout(function(){
							thisNavList.fadeOut(100);
							jti.removeClass("jt");
							thisNavList.find("span").removeClass("t");
						},100);
					})
				}
            });
			...
			$("header .login #login").on("click",function(event){
				event.preventDefault();
				$.dxlLoginPage();
			});
			$("header .login #favorite").on("click",function(event){
				event.preventDefault();
				$.dxlLoginPage($(this).attr("href"));
			});
		...
		"dxlAssociation":function(def){
			//搜索
			var ci = $.cookie("city") ? $.cookie("city") : "sh";
			var par = {
				"input":$("header #topHeader .search .kuang input"),//搜索框
				"label":$("header #topHeader .search .kuang label"),//提示语句
				"btn":$("header #topHeader .search .kuang span"),//搜索按钮
				"ass":$("#ass")//显示层
			}
			$.extend(par,def);
			par.input.val("");
			
			par.label.on("click",function(){
				par.input.focus()
			})
			par.input.on({
				"focus":assShow,
				"blur":function(){assHide();par.label.removeClass("cur");},
				"keyup":assShow
			});
			par.btn.on("click",btnClick);
			par.ass.find("div").on("click",function(){
				par.btn.click();
			});
			
			function btnClick(){
				window.location = dxlHttp.newCom + ($.trim(par.input.val()) != "" ? "search?q=" + encodeURIComponent($.trim(par.input.val())) : "HunYan/");
			}
			
			function assShow(event){
				if(event.keyCode == 13){
					btnClick();
				}
				par.label.addClass("cur");
				$.trim(par.input.val()) != "" ? par.label.hide() : par.label.show();
				
				$.getJSON(dxlHttp.com + "jsonpnew/?act=getSearchAutoComplete&q=" + encodeURIComponent(par.input.val()) + "&city=" + ci + "&callback=?",function(data){
					if(data.code == 1){
						var html = "";
						for(i=0 ; i < data.search_hotel.hotel.length ; i++){
							html += '<li url="'+data.search_hotel.hotel[i].url+'">'+data.search_hotel.hotel[i].name+'</li>';
						}
						par.ass.find("ul").html(html);
						par.ass.find("div b").text(data.search_hotel.count);
						par.ass.slideDown(100);
					}else{
						assHide();
					}
					par.ass.find("ul li").on("click",function(){
		...
		"dxlHeadCity":function(){
			//城市高亮
			var cityList = $("#cityList");
			$("header .city span").on("click",function(e){
				e.stopPropagation();
				var _this = $(this);
				cityList.css({"top":_this.offset().top + _this.outerHeight(),"left":_this.offset().left}).fadeIn(300);
			});
			$(document).on("click",windowClose);
			$(window).on("resize",windowClose);
			...
			/*  选中城市高亮   */
			var cityLi = $("#cityList li");
			for(var i=0; i<cityLi.length; i++){
				if(cityLi.eq(i).attr("city")==$.cookie("city_id")){
					cityLi.eq(i).addClass("cur");
				}
			}		

		},
		"dxlLinkSlider":function(){
			// 友情链接滚动
			var def = {
				"target1":".friendLink .wAuto",
				"target2":".hotLink .wAuto",
				"speed":50,
				"con1":".list1",
				"con2":".list2",
				"con3":".list3",
			...
			var box1 = $(def.target1),box2 = $(def.target2);
			var con1 = $(def.con1),con2 = $(def.con2),con3 = $(def.con3),con4 = $(def.con4);
			con2.html(con1.html());
			con4.html(con3.html());
			function Marquee1(){
				if(box1.scrollLeft()>= con1.width()){
					box1.scrollLeft(0);
				}
				else{
					box1.scrollLeft(box1.scrollLeft()+1);
				}	
			}
			
			function Marquee2(){
				if(box2.scrollLeft()>= con3.width()){
					box2.scrollLeft(0);
				}
				else{
					box2.scrollLeft(box2.scrollLeft()+1);
				}	
			}
			
			var sliding1 = setInterval(Marquee1,def.speed);
			var sliding2 = setInterval(Marquee2,def.speed);
			box1.on("mouseover",function(){
				clearInterval(sliding1);
			}).on("mouseleave",function(){
				sliding1 = setInterval(Marquee1,def.speed);
			});
			
			box2.on("mouseover",function(){
				clearInterval(sliding2);
			}).on("mouseleave",function(){
				sliding2 = setInterval(Marquee2,def.speed);
			});					
		},
		"dxlgetStar":function(){
			/*星星*/
			$(".dxlxx").each(function(index, element) {
				$(element).append("<span></span>");
				var _this = $(element).find("span");
				var total = parseFloat($(element).attr("total")) * 20;
				//_this.animate({"width":total + "%"},2000);
				_this.css("width",total + "%");
			});
			
		},
		"dxlRecording":function(){
			/*  浏览记录  */ 
			var record = $("#record");
			$(".login .rec").on({"mouseenter":mouseenterNav,"mouseleave":mouseleaveNav});
			record.on({"mouseenter":mouseenterNav,"mouseleave":mouseleaveNav});
			  
			 
			var reTime;
			function clearTime(callback){
				record.stop();
				window.clearTimeout(reTime);
				callback();
			}
	...
	$.fn.extend({
		"dxlGetFocus":function(param){
			var $this=$(this),
				$label_obj=$this.parent().find(param.label_obj);
			...
			$this.on(
				...
				function(){
					var $_this=$(this);
						$label_obj=$_this.parent().find(param.label_obj);
				}
			);
			...
			$this.on(
				"keyup",
				function(){
					var val_inp=$.trim($(this).val());
					if(val_inp){
						$label_obj.fadeOut(200);
						$(param.msg_obj).text("").removeClass("msg_phone_err").removeClass("msg_phone_ok");
					}else{
						$label_obj.fadeIn(200);
					}
				}
			).focus();
			return $this;
		},
		"dxlPositionFixed":function(callback){
			if(!$(this).length){
				return;
			}else{
				var $this=$(this),
					tabs_top=$this.offset().top;
			}
			$(window).on(
				"scroll",
				function(){
					if(dxlHttp.ie){return;}
					var $_this=$(this),
						$blank=$("#blank");
					$blank.css({"width":$this.outerWidth(),"height":parseInt($this.outerHeight(true),10)});
					if($_this.scrollTop()>=tabs_top){
						$("#blank").show();
						$this.addClass("head_fixed");
						
					}else{
						$("#blank").hide();
						$this.removeClass("head_fixed");
		...
		"dxlDoRating":function(param){
			var $this=$(this);
			$this.each(
				function(){
					var $_this=$(this),
					$obj_cur=$_this.children("b"),
					w_total=parseInt($_this.width(),10),
					val_cur=parseFloat($.trim($obj_cur.text()),10),
					w_cur=(val_cur*w_total)/param.total_score;
					$obj_cur.css({"width":w_cur});
				}
			);
		},
		"dxlDoTextarea":function(param){
			var $this=$(this);
			if($.trim($this.val()).length){
				$(param.area_label).hide();
			}
			$this.on(
				"keyup",
				function(){
					var area_len=Math.ceil(($.trim($this.val()).replace(/[^\x00-\xff]/g, "**").length)/2);
					if(area_len){
						$(param.area_label).hide();
						if(area_len<param.text_min){
							$(param.msg_obj).html('<span class="msg_area_err">最少20个字，你还需要输入<b class="numb_input_w">'+(param.text_min-area_len)+'</b>字</span>');
						}
						if(area_len>=param.text_min&&area_len<=param.text_max){
							$(param.msg_obj).html('<span class="msg_area_ok">已输入<b class="numb_input">'+area_len+'</b>字，还可以输入<b class="numb_input_t">'+(param.text_max-area_len)+'</b>字</span>');
						}
						if(area_len>param.text_max){
							$(param.msg_obj).html('<span class="msg_area_err">已输入<b class="numb_input">'+area_len+'</b>字，超出了<b class="numb_input_w">'+(area_len-param.text_max)+'</b>字</span>');
						}
					}else{
						$(param.area_label).show();
						$(param.msg_obj).html('<span class="msg_area_err">最少20个字，你还需要输入<b class="numb_input_w">'+(param.text_min-area_len)+'</b>字</span>');
					}
				}
			);
		...
		"dxlDeleteHistory":function(param){
			var $this=$(this),
				img_url=dxlHttp.i+"hotel/170x106/",link_url=dxlHttp.newCom+"HunYan/";
			$this.on(
				"click",
				param.btn_detele,
				function(){
					var $_this=$(this),id_all=[],
						id_delete=$_this.data("hotel_id"), status=$_this.data("delete"),
						html="";
					if(status==="disabled"){
						return;
					}
					$_this.data("delete","disabled");
					$(param.btn_detele).each(
						function(){
							id_all.push($(this).data("hotel_id"));
						}
					);
					$.getJSON(
						dxlHttp.com+"jsonpnew/?act=delCookieRecentView"+"&hotel_id="+id_delete+"&id_arr="+id_all.join("|")+"&callback=?",
						function(data){
							if(data.code===1){
								$_this.parent().remove();
									html+="<li>";
									html+='<a class="rec_hotel_img" href="'+link_url+data.data[0].url+'-Info'+'" rel="nofollow" target="_blank"><img alt="'+data.data[0].name+'" src="'+img_url+data.data[0].img[0].path+'" /></a>';
									html+='<div class="wrap_hotel_location clear_float"><span class="rec_hotel_name"><a href="'+link_url+data.data[0].url+'-Info'+'" target="_blank" title="'+data.data[0].name+'">'+data.data[0].name+'</a></span><span class="hotel_location">'+data.data[0].region+'</span></div>';
									html+='<span class="history_delete" data-hotel_id="'+data.data[0].id+'">删除</span>';
									html+="</li>";
									$this.find(param.box_list).append(html).show();
							}else if(data.code===0&&$this.find(param.box_list).children("li").length>1){
								//无数据返回，不是最后一个
								$_this.parent().remove();
							}else if(data.code===0&&$this.find(param.box_list).children("li").length===1){
								//无数据返回，且为最后一个
								$this.hide();
								$(param.other_obj).hide();
							}else{
								//删除失败，恢复到原始状态
								$_this.data("delete","enabled");
							}
						}
					);
				}
		...
		"dxlAddCollection":function(callback){
			var $this=$(this);
			$this.off("click.fav");
			$this.on(
				"click.fav",
				function(){
					$.dxlmg({"order_from":$.cookie("city") + "_hunyanshoucang"}); 
					var status=$this.data("collect");
					if(status==="disabled"){
						return;
					}
					$this.data("collect","disabled");
					var hotel_param=$this.data("param").split("|"),
						left=$this.offset().left,
						top=$this.offset().top-28,
						base_url=window.location.protocol+"//"+window.location.host+"/";
					if($this.is(".collect_btn_add")){
						$.dxlCollection({
								id:hotel_param[0],
								name:hotel_param[1],
								url:hotel_param[2]
								...
								if(data.code===1){
									$this.removeClass("collect_btn_add").addClass("collect_btn_cancel").text("已收藏").attr("title","取消收藏").data("collect","enabled");									
								}else{
									$this.text("收藏").data("collect","enabled");
								}
							}
						);
						
					}else if($this.is(".collect_btn_cancel")){
						$.getJSON(
							dxlHttp.com+"jsonpnew/?act=delFavoriteHotel"+"&id="+hotel_param[0]+"&name="+encodeURIComponent(hotel_param[1])+"&url="+hotel_param[2]+"&callback=?",
							function(data){
								if(data.code===1){
									$this.removeClass("collect_btn_cancel").addClass("collect_btn_add").text("收藏").attr("title","点击收藏").data("collect","enabled");
								}else{
									$this.text("已收藏").data("collect","enabled");
								}
							}
						);	
					}	
