...
Ext.XTemplate = function(){
 ...
 // var re = /<tpl\b[^>]*?>((?:(?=([^<]+))\2|<(?!tpl\b[^>]*>))*?)<\/tpl>/;
 // var re = /<tpl\b(?:(?:\s*(?:if|for|exec)\s*"(.*?)"\s*)|[^>]*)>((?:(?=([^<]+))\2|<(?!tpl\b[^>]*>))*?)<\/tpl>/;
    
   var re = /<tpl\b\s*(?:if|for|exec)?\s*=?\s*(?:\"[^"]*\")?\s*>((?:(?=([^<]+))\2|<(?!tpl\b[^>]*>))*?)<\/tpl>/; 
   ...
   // var re =/<tpl\b[^>]*>((?:(?=([^<]+))\2|<(?!tpl\b[^>]*>))*?)<\/tpl>/;
	...
	var nameRe = /^<tpl\b[^>]*?for\s*=\s*"(.*?)"/;
    var ifRe = /^<tpl\b[^>]*?if\s*=\s*"(.*?)"/;
    var execRe = /^<tpl\b[^>]*?exec\s*=\s*"(.*?)"/;
