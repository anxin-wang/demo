...
Ext.extend(Morik.Office.MessagePanel, Ext.Panel, {
	...
	afterRender : function(ct) {
		Morik.Office.MessagePanel.superclass.afterRender.apply(this, arguments);
		this.list = new Ext.ux.BalloonTip( {
			direction : 'l',
			yDetal : 3,
			xDetal : 0,
			xDetalRate : 80,
	...
	showMsg : function(d, i, el, event) {
		if (this.tipHide == true) {
			var msg = this.msgTpl.applyTemplate(d.store.getAt(i).data);
			if (event)
				this.list.show(msg, event.getXY());
			else {
				var xy = this.menu.getEl().getXY();
				xy[0] = xy[0] + this.menu.getEl().getWidth() + 2;
				xy[1] = xy[1] + 5;
				this.list.show(msg, xy);
			}
			this.tipHide = false;
		}
	}
